...
# opt.disable_colorization = False
...
with torch.no_grad():
    ...
    while True:
        ...
        # print("output.l.min: ", output_l.min(), "max: ", output_l.max())
        # # Perform restoration
        # input.fill_(0.0)
        # output_l = modelR(input)  # [B, C, T, H, W]
        # print("0 ==> ", output_l.min(), output_l.max())
        # input.fill_(1.0)
        # output_l = modelR(input)  # [B, C, T, H, W]
        # print("1.0 ==> ", output_l.min(), output_l.max())
        # pdb.set_trace()
        #  input.min:  tensor(0., device='cuda:0') max:  tensor(0.9765, device='cuda:0')
        #  output.l.min:  tensor(0., device='cuda:0') max:  tensor(0.9815, device='cuda:0')
        #  0 ==>  tensor(0., device='cuda:0') tensor(0.0372, device='cuda:0')
        #  1.0 ==>  tensor(0.7527, device='cuda:0') tensor(1., device='cuda:0')

        # (Pdb) input.size(), output_l.size()
        # (torch.Size([1, 1, 5, 320, 432]), torch.Size([1, 1, 5, 320, 432]))
        ...
        # Perform colorization
            ...
            # pdb.set_trace()
            # (Pdb) output_l.size(), output_ab.size()
            # B,C,T,H,W
